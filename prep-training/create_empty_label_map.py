...
def create_empty_label_map(input_dir, reference_filename, output_filename):
    ...
    for root, dirs, files in os.walk(input_dir):
        for subdir in dirs:
            ...
            output_file_path = os.path.join(subdir_path, output_filename)
            if not os.path.exists(output_file_path):
                # Check if the reference file exists
                reference_file_path = os.path.join(subdir_path, reference_filename)
                if os.path.exists(reference_file_path):
                    # Load the reference file to get its shape and affine
                    ...
                    empty_data = np.zeros_like(reference_data)
